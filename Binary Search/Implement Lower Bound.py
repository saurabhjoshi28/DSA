class Solution:
    def lowerBound(self, arr, target):
        ...
        # → arr[3] = 5 is the first ≥ 4 → return 3
        #
        # arr = [1, 2, 3], target = 5
        ...
        # the element is greater than or equal to the target.
        #
        # We initialize:
        #   - low = 0 (start of search space)
        #   - high = n - 1 (end of search space)
        #   - lower_bound = n (default answer if not found)
        #
        # At each step:
        #   - if arr[mid] >= target:
        #       → it's a **candidate**, but there may be a smaller one to the left.
        #       → so we update lower_bound = mid, and move high = mid - 1
        #   - else:
        ...
        n = len(arr)
        low, high = 0, n - 1
        lower_bound = n  # default value if no element ≥ target

        while low <= high:
            mid = (low + high) // 2

            # Candidate found, try to find earlier one
            if arr[mid] >= target:
                lower_bound = mid
                high = mid - 1
